...
- _Note:_ Short-retrieval: ~3 concise sentences in own words, no quotes; marked by counting valid distinct points against the mark scheme. No TTECEA/planning. NOTE: protocols/shared/language-paper-specs.json lists Q2 as 4 marks — the live protocol (this file) states 3 marks and sums (2+3+6+12+22=45) to match Section A's total; treated the protocol as canonical per WML CLAUDE.md ('protocol-a-assessment.md is the definitive marking document'). Flagging the spec-JSON/protocol mismatch, not resolving it here.
...
- **6 marks** · AOs: AO1 · structure: **SKIP (none)**
- _Note:_ Mark-per-sentence retrieval/interpretation (6 simple sentences with brief quotes, 1 mark each) — not TTECEA paragraphs, no planning taught. NOTE: protocols/shared/language-paper-specs.json lists Q3 as 5 marks with AOs [AO1, AO2] ('short_analysis' type) — the live protocol states 6 marks, AO1 only, pure retrieval type, and its own Total-Mark line (/6) is internally inconsistent with its Level-alignment prose which twice says '/5' (line 474: 'Your [X]/5'). Both discrepancies (spec-vs-protocol marks/AOs, and the protocol's own internal 6-vs-5 typo) are flagged, not resolved, here — used the protocol's 'Total Mark for Q3: [X]/6' as canonical since Section A's total (2+3+6+12+22=45) only reconciles at 6.
...
- _Note:_ This is the SAME TTECEA rows adapted for comparison (per feedback_comparative_body_is_ttecea_helper_text_only memory), not a different skeleton, with the two reader-effect rows generalised to 'reader impact 1/2' rather than one-per-source. ARITHMETIC FLAG: the 8 body elements listed in the live protocol sum to 5.0 marks, but the paragraph is labelled '(6 Marks each)' and the essay total (Intro 2 + Body 3×6 + Conclusion 2 = 22) only reconciles using 6, not the actual 5.0 the listed criteria add up to — a 0.5-mark/paragraph shortfall (×3 = 1.5 marks) in the live protocol's own element list. Flagging per file:line, not correcting.

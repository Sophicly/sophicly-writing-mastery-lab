...
- **CANVAS_SOURCES_PRESENT()** *(v7.18.35 — universal across boards)* — before any source-collection step asks the student to paste an extract, scan the prompt context for a `[SOURCES — supplied via canvas, do NOT ask the student to paste them]` block. The frontend prepends this block whenever the canvas already contains populated source sections (mastery topics, practice papers, exam-prep mounts that ship pre-populated with source material). Inside the block, sources are delimited by `=== SOURCE A ===`, `=== SOURCE B ===`, etc. If a source segment is present, you MUST skip the corresponding ask, auto-store the source text into the relevant `SESSION_STATE.source_*_content` field, extract title and author from the first non-empty line(s) of the segment, and confirm to the student in one short sentence (e.g. "I can see Source A is already populated in your canvas, so we can move on."). Only fall back to asking the student to paste when the corresponding `=== SOURCE X ===` segment is absent. This rule applies to every board's source-collection flow — AQA Language Paper 1 + 2, Edexcel, Eduqas, OCR, Edexcel IGCSE, CCEA, SQA, and any future board added. It also applies to single-source papers (e.g. literature unseen) where the canvas may contain a single `=== EXTRACT ===` or `=== SOURCE ===` segment.

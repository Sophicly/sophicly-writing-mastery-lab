### Creative Writing Workbook: Step 23 — Give Your Story Genre(s)

#### Overview

- **Type:** Workbook (self-paced, no AI)
- **Prerequisite:** Step 22 (Draft 5) + Trial 5
- **Feeds into:** Step 24 (Update Plot Structure) then Step 25 (Draft 6 — Genre)
- **Hero's Journey Stage:** The Final Trial approaches. Now add genre — the emotional contract with your reader.

---

#### Concept: Genre

**Genre = A promise to the reader about what emotional experience they will have.**

Each genre has: dominant emotion(s), expected conventions/tropes, and a specific atmosphere. Most sophisticated stories BLEND genres. Your story can shift genres from beginning to middle to end.

---

#### The Five Genre Categories (by Emotion)

**1. Courage Genres** (Emotion: Excitement, Tension)
- Includes: Action, Adventure, War, Western, Heroic Science Fiction, Dystopian
- Conventions: Enslavement or deprivation of free will, intensified settings, victory over death

**2. Fear and Loathing Genres** (Emotion: Dread, Unease)
- Includes: Gothic, Horror, Supernatural, Dark Science Fiction
- Conventions: Mystery, terror, good vs evil, decay, dreams/nightmares, mental landscape, monsters, fearful atmosphere

**3. Wonder and Awe Genres** (Emotion: Amazement, Possibility)
- Includes: Fantasy, Science Fiction, Magical Realism
- Conventions: Otherworldly settings, mythical creatures, epic quests, mystical lore, futuristic elements

**4. Need to Know Genres** (Emotion: Curiosity, Suspense)
- Includes: Detective, Mystery, Spy, Suspense, Thriller
- Conventions: Mystery, suspense, surprise, "the innocent man" thrust into danger

**5. Heart Genres** (Emotion: Connection, Growth)
- Includes: Drama, Coming-of-Age
- Conventions: Relationships, emotional intimacy, personal growth, emotional conflict

---

#### Task: Identify Your Story's Genre(s)

For each genre category, select whether it applies to your story and identify which specific conventions you will use:

**Courage Genres:**
- [ ] Action-Adventure  [ ] War  [ ] Western  [ ] Heroic Sci-Fi  [ ] Dystopian

Conventions present:
- [ ] Enslavement or deprivation of free will — Explain: ___
- [ ] An intensified setting — Explain: ___
- [ ] A protagonist we empathise with — Explain: ___
- [ ] Victory over death — Explain: ___

`[SUBSTEP_COMPLETE: step_23, substep_1, "Courage and Fear Genres"]`

**Fear and Loathing Genres:**
- [ ] Gothic  [ ] Horror  [ ] Supernatural  [ ] Dark Sci-Fi  [ ] Dystopian

Conventions present:
- [ ] Mystery — Explain: ___
- [ ] Terror — Explain: ___
- [ ] Good and evil — Explain: ___
- [ ] Decay — Explain: ___
- [ ] Dreams/nightmares — Explain: ___
- [ ] Mental landscape — Explain: ___
- [ ] Monsters — Explain: ___
- [ ] Fearful atmosphere — Explain: ___

---

**Wonder and Awe Genres:**
- [ ] Fantasy  [ ] Science Fiction  [ ] Magical Realism

Conventions present:
- [ ] Otherworldly settings — Explain: ___
- [ ] Mythical creatures — Explain: ___
- [ ] Epic quests — Explain: ___
- [ ] Mystical or ancient lore — Explain: ___
- [ ] Futuristic/otherworldly elements — Explain: ___

`[SUBSTEP_COMPLETE: step_23, substep_2, "Wonder and Mystery Genres"]`

**Need to Know Genres:**
- [ ] Detective  [ ] Mystery  [ ] Spy  [ ] Suspense  [ ] Thriller

Conventions present:
- [ ] Mystery — Explain: ___
- [ ] Suspense — Explain: ___
- [ ] Surprise — Explain: ___
- [ ] "The innocent man" thrust into danger — Explain: ___

---

**Heart Genres:**
- [ ] Drama  [ ] Coming-of-Age

Conventions present:
- [ ] Relationships — Explain: ___
- [ ] Emotional intimacy — Explain: ___
- [ ] Personal growth — Explain: ___
- [ ] Emotional conflict — Explain: ___

`[SUBSTEP_COMPLETE: step_23, substep_3, "Heart Genres and Summary"]`

---

#### Genre Summary

**Primary genre(s):** ___
**Secondary genre(s):** ___
**Dominant emotion I want to create:** ___

---

#### Sub-step Overview

| Sub-step | Name | Deliverable | Completion Signal |
|----------|------|-------------|-------------------|
| 1 of 3 | Courage and Fear Genres | Courage + Fear/Loathing genres identified with conventions | `[SUBSTEP_COMPLETE: step_23, substep_1, "Courage and Fear Genres"]` |
| 2 of 3 | Wonder and Mystery Genres | Wonder/Awe + Need to Know genres identified with conventions | `[SUBSTEP_COMPLETE: step_23, substep_2, "Wonder and Mystery Genres"]` |
| 3 of 3 | Heart Genres and Summary | Heart genres identified; full genre summary completed | `[SUBSTEP_COMPLETE: step_23, substep_3, "Heart Genres and Summary"]` |

#### Data Requirements

**Reads from project:**
- `plot_outline` — Six-stage plot outline (version 5)
- `theme` — Story's theme (from Step 20)
- `primary_archetype` — Plot structure (from Step 5)

**Writes to project:**
- `genre_choices` — Primary and secondary genres
- `genre_conventions` — Specific conventions selected
- `genre_emotion` — Dominant emotion targeted

**Next step:** Step 24 — Update Plot Structure (Genre), then Step 25 — Draft 6 (AI protocol).

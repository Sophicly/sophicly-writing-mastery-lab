# [AI_INTERNAL — CODE-SERVED SOURCE] CW Step 3 — byte-diff source

**NOT loaded by any manifest and must never be** (leading `_`, absent from every manifest.json).
Byte-diff source for the strings `_cwLoglineCtl` serves from code. Teaching text left in a loaded
module gets narrated by Sophia regardless of any fence — WML CLAUDE.md #5.
...
You now have all seven building blocks. Here they are, in your own words:

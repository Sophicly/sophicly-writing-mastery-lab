# The War of the Worlds — Concept Notes (autofilled into the Conceptual Notes doc on FQ mastery)

Companion to `war_of_the_worlds.md`. NOT a quiz bank. One pre-authored concept note per aspect, filed
into the Conceptual Notes document when a student answers that aspect's question correctly, matching the
five FQ dimensions and the lit CN doc fields via `concept_field_for_dim()`:
...
### Message
- **Note:** The novel warns that mankind's complacent belief in its own mastery — and the imperial cruelty that flows from it — is fragile and self-condemning: saved by microbes rather than its own power, humanity is humbled, small and vulnerable within an indifferent cosmos, and called to humility rather than pride.

# Rubric — Universal Base

**Used by:** ALL per-paper rubric modules. Each per-paper override ASSUMES this base is loaded and ADDS paper-specific criteria on top.
...
## TOPIC SENTENCE — STRICT CONCEPT-ONLY RULE

Sentence 1 of every BP = pure concept. Zero techniques. Zero forms.

Do not name the form (no "fractured sonnet" / "dramatic monologue") in the topic sentence. Techniques + evidence + inference go in sentence 2.

Every body paragraph has ONE anchor quote that holds the main concept.

**Pointer phrase:** *Topic sentences name the concept, not the technique.*

(Per memory `feedback_topic_sentence_concept_not_technique`.)
...
(Per memory `feedback_crib_sheets_anchor_quotes` — every analytical claim anchored.)

---

## VOCABULARY PRECISION — BANNED VERBS

Banned analytical verbs. Universal GCSE fallback. Force lazy analysis.

**Banned:** `shows`, `is`, `has`, `gets`, `does`.
...
- No colloquialisms (`kind of` / `pretty` / `just` / `stuff`).
- No chatty asides (`obviously` / `surely`).
- No first-person intrusions (`I think` / `we can see`).
...
(Per memory `feedback_essays_always_academic_register`.)

---

## SENTENCE LENGTH CAP

Sentences **≤ 35-45 words** (≤ ~3 lines). Longer sentences typically split or trim.

Reason: 13-16 year-olds cannot follow longer sentences. Examiners likewise prefer clarity. Academic register does NOT mean obscure.

**Pointer phrase:** *Sentences over ~35 words usually carry weight that two short sentences would carry better.*

(Per memory `feedback_sentence_length_max_three_lines`.)

---

## STRUCTURE RULES

- **No arrow characters** (→, ->, =>) in essay prose OR plan bullets. Use prose connectors (*through* / *from* / *then* / *leading to* / *which produces*).
- **No form-naming in topic sentence.** Forms appear in sentence 2.
- **Show extracted Before/After** when revising — student articulates the diff.

(Per memory `feedback_no_arrows_in_essays`, `feedback_topic_sentence_concept_not_technique`, `feedback_show_extracted_edits`.)
...
- Audience = examiner (analytical writing) OR specified audience (transactional / creative).
- NEVER "reader" — always "audience" for drama, "reader" for prose, "listener" for transactional speeches. Match register to text-type.
- (Per memory: audience-not-reader for plays.)

---

## NAMED VOICES IN STUDENT WORK

- Don't write "Neil" / "Abdullah" / any user name into shipped student-facing prose. Depersonalise as "the writer" / "this guide" / "the piece". (Per memory `user_name_in_documents`.)
- Don't introduce explicit sexual references in student materials. Reframe Victorian-context references in legal-status / social-vulnerability terms. (Per memory `feedback_no_sexual_references_in_student_materials`.)

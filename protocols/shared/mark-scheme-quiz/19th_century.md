...
1. **Phase 1 step 3 (Ready Gate emission turn):** BEFORE emitting the Ready Gate text, call `quiz_start('mark_scheme', 5, '[selected_board lowercase]', '19c_novel', 1)`. Use the exact board slug. Attempt_number is `1` for first attempt; higher if Phase 4 retry.
...
5. **Phase 4 "Try another round":** Call `quiz_start('mark_scheme', 5, '[board]', '19c_novel', attempt_number+1)` SILENTLY before re-entering Phase 2.

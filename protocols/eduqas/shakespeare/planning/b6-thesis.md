...
IF still incorrect → Increment to Hint Band 3
...
IF still says they can't find it/don't have workbook → Increment to Hint Band 4
...
IF still incorrect → Increment to Hint Band 5
...
IF still incorrect → Return to Hint Band 3 (workbook reference) with more direct instruction
...
IF B → "Alright. Let's try again with fresh perspective. For Body \[X\], you were exploring..." → Return to Hint Band 2 with slightly different wording
...
- If thesis off-topic/descriptive: ask one micro-question (e.g., "What is your claim about \[focus\]?") and micro-nudge (e.g., "Try assertive verb instead of *shows* to reach Band 5 precision")

<!-- MODULE: Protocol B: B.4 Anchor Quotes (B/M/E) -->
...
* **If question includes an extract (Option A from Step 4):** At least one anchor must come from the given extract (students may place it in any paragraph). Do not force quotes into the introduction unless used as a hook.  
* **If question is without extract (Option B from Step 4):** All three B/M/E anchors should come from across the whole text as usual. No extract constraint applies.

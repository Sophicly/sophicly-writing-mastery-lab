...
**\[AI\_INTERNAL\] ROUND ONCE — the round-once discipline, stated (B-CHECKS `NEVER round`; PROTOCOL-STANDARD A6):** **NEVER round a section total.** A `Total Mark for [section]` line is the plain decimal sum of that section's criteria minus its penalties — no "→ rounded" suffix, no "Base total:" line, no re-derivation. Rounding happens **exactly ONCE**, half-up, at the `Total: X/30` Final Total line, and the Final Total is the sum of the five DECIMAL section totals (ceiling applied per the word-count rule above). The engine re-computes and re-bands both anyway — the code is the net, not the excuse. Every %/grade line, per-section and final, bands on the CANONICAL GRADE LADDER stated in this protocol — never real-exam boundaries.

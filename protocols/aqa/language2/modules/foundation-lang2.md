...
*(Version changelog removed v7.19.854 — see git history. This file carries LIVE content only.)*
...
HEAL OLD PAPERS: if a stored or displayed Q2 question still uses the old wording ("Write a summary of the differences/similarities…"), present it in its updated 2026 form instead, matching AQA's authentic three-sentence shape: "You need to refer to Source A and Source B for this question. The writers in Source A and Source B \[one sentence naming the shared topic\]. What can you infer about the differences between \[same topic, short noun phrase\]?" Add one short note: "(Your paper shows the older question style \- the exam board has updated Q2, so we'll work in the current inference format.)" Then continue normally. Do NOT add an instruction about quotations to the question itself \- the quotation requirement comes from the mark scheme and is taught in the walk.

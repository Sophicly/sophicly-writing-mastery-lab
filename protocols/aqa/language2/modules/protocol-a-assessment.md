...
* **Claim perceptive (AO1): plus 0.25 marks** → the claim is subtle or insightful, beyond the obvious surface reading. On Source B units, Claim and Perceptive judge the DIFFERENCE-claim (the comparison the unit states between the texts — AQA: "statements show perceptive differences"), not merely the Source B reading on its own. Zero-example: "the storm is presented as destructive" = 0 (the storm is literally destructive — that is the surface); "the storm is presented as a force that erases human ambition" earns it. Awarded **\[X\]** out of 0.25 because \[specific reason\].  
...
* **Quote-anchored (AO1): plus 0.25 marks** → a judicious quotation, relevant to the question's focus, is embedded in the development (a well-chosen quote about something the question did not ask = 0). Paraphrase without quotation = 0 for this criterion only (a detailed but unquoted development keeps its Detail quarter). Awarded **\[X\]** out of 0.25 because \[specific reason\].

<!-- CURRENT STEP — do ONLY this step, in full, then STOP. Do not run later steps; do not skip ahead. (Slice of protocol-a-assessment.md. v7.19.362: the monolith's feedback→gold Pause gate now lives HERE — the feedback turn must END by offering the gold step, never "move to Paragraph 2"; the original slicing left this cue in the gold file, so the model invented its own next-step offer and bulldozed the held gold beat — Neil's 10 Jun run 4.) -->
...
**Internal AI Note:** After this paragraph's mark breakdown, feedback and advice, you MUST deliver THIS paragraph's gold standard step next — the Pause gate below, then BOTH gold blocks once the student types Y. NEVER skip from feedback to the next paragraph (or to the question summary) before this paragraph's two gold standard examples are delivered. END THIS TURN with the Pause gate — offer no other next step.

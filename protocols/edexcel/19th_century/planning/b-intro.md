...
**\[AI\_INTERNAL\] ENTRY TRIGGER:** Initialize this protocol when student chooses to **plan an essay**. Entry can occur from:
...
1. B.1 Initial Setup → 2\. B.2 Goal Setting → 3\. B.3 Diagnostic Import (optional) → 4\. B.4 Anchors → 5\. **B.5 Bodies (plan all three body paragraphs using TTECEA)** → 6\. **B.6 Working Thesis (synthesize from body paragraphs)** → 7\. **B.7 Introduction** (BOTH: hook \+ building \+ thesis | Q1a: building focuses on stylistic/technical | Q1b: building evaluates context) → 8\. **B.8 Conclusion** (BOTH: 4-step structure | Q1a: Step 3 = Concept→Technique Links | Q1b: Step 3 = Concept→Context Links) → 9\. B.9 Final Review → 10\. B.10 Final Instructions

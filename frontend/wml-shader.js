/**
 * SWML Shader Background — Stripe-like gradient
 * Extracted from Framer component, recoloured for Sophicly brand.
 * Pure WebGL — no Three.js or other dependencies.
 */
(function() {
    'use strict';
...
`;
    ...
    // ─── Config ───
    // Type: 0=linear, 1=conic, 2=animated(Stripe), 3=wave, 4=silk, 5=smoke, 6=stripe
    const CONFIG = {
        color1: [83/255, 51/255, 237/255],     // #5333ed — Sophicly purple
        color2: [81/255, 218/255, 207/255],     // #51dacf — Sophicly teal
        color3: [44/255, 0/255, 62/255],        // #2c003e — Sophicly deep purple
        speed: 0.4,
        scale: 1.0,
        type: 6,       // stripe
        noise: 0.08,
    ...
    let gl, program, animId, container;
    let startTime = 0;
    let uniforms = {};
    ...
    function init(containerEl) {
        ...
        if (!gl) {
            console.warn('SWML Shader: WebGL not supported');
            return false;
        }
        ...
        if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
            console.warn('SWML Shader: Link failed', gl.getProgramInfoLog(program));
            return false;
        }
        ...
        ['u_time','u_color1','u_color2','u_color3','u_speed','u_scale','u_type','u_noise','u_resolution'].forEach(name => {
        ...
        gl.uniform1i(uniforms.u_type, CONFIG.type);
    ...
    function compileShader(gl, type, source) {
        ...
        if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
            console.warn('SWML Shader compile error:', gl.getShaderInfoLog(shader));
            return null;
        }
        return shader;
    }
    ...
    function resize() {
        ...
        if (uniforms.u_resolution) {
            gl.uniform2f(uniforms.u_resolution, w * dpr, h * dpr);
        }
    }
    ...
    window.swmlShader = { init, destroy, resize };
})();

...
class SWML_Protocol_Router {
    ...
    private function legacy_essay_assessment_preamble() {
        ...
        $p .= "### WORD-COUNT CEILING — SETUP STAGE ONLY, CEILING NOT DEDUCTION\n";
        $p .= "State any word-count ceiling + the adjusted maximum ONCE, at the setup stage (per the protocol), tied to the student's grade target. Do NOT restate or introduce it inside a section's feedback card. v7.19.816 (Neil, SETTLED): the WC penalty is a CEILING, never a deduction — Final Total = MIN(sum of the five section totals, max − WC penalty). NEVER subtract it from the section sum; under-length essays already lose marks organically.\n\n";
        ...
        $p .= "Grade = highest band the percentage reaches on Sophicly's canonical ladder: 9≥85%, 8≥75%, 7≥65%, 6≥55%, 5≥45%, 4≥35%, 3≥25%, 2≥15%, else 1 (0 = not answered). Work the boundary check silently; never pattern-match a grade or accept a student override. This SAME ladder bands EVERY grade you output — including each section's 'Percentage & Grade' line — NEVER real-exam boundaries (live 2026-07-02: sections banded 33%→2 and 38%→3 off the real-exam scale while the final used the canonical ladder — two scales in one assessment).\n\n";
    ...
    private function build_poetry_cn_injection($context) {
        ...
        if ($sel_id !== '') {
            ...
            if ($sel) {
                ...
                // v7.19.995 (§A16): the speaker/form/purpose FIRST-QUESTIONS are now asked by the
                ...
                $b .= "Skip the picker — walk THIS poem. The student has ALREADY chosen it; the full text is below. Their message USUALLY arrives WITH their speaker stance and reasoning (an @ELEMENT_STANCE line — the interface asked the designed question with buttons); respond DIRECTLY to that stance Socratically (probe their evidence, develop it conceptually). If INSTEAD their message says they are CONTINUING or REVISITING a poem with elements already filed (no stance line), do exactly what it directs: resume the walk at the element it names; or deepen the filed element it asks to revisit (recap their existing note in 1–2 lines FIRST, then ONE targeted question — and if the note is already strong, say so honestly and recommend moving on to an unfilled element); or, when it names ONE empty box (quotes / effect), work ONLY on that box and file ONLY that field. Never restart at speaker uninvited, never re-walk elements or boxes the message didn't name. Either way: do NOT greet again, do NOT ask them to \"say ready\", do NOT re-ask who is speaking, do NOT re-present the picker. Likewise the interface asks the FIRST question of the form and purpose elements: after filing context, bridge one line to Form and END your message; after filing themes, bridge one line to Purpose and END your message. File every element into these EXACT fieldIds:\n";
    ...
    private function load_modular_protocol($context, $user_id = 0) {
            ...
            if (file_exists($msu_path)) {
                ...
                if ($step !== 2) {
                    $content .= "\n\n---\n\n## ⛔ ABSOLUTE OUTPUT RULE — READ LAST, OBEY FIRST (overrides everything above)\n\n"
                        ...
                        . "The RESULTS turn — only AFTER the student answers Q5 and asks for results — is the FIRST time you reveal anything: emit `[[QUIZ_DONE]]` on its own line, then for EACH of the 5 questions give its ✓/⚠️/✗ mark, the correct answer, and the explanation/exemplar from the bank — all in one message.\n\n"
                        . "### MARKER INTEGRITY (the score depends on this)\n"
                        . "Emit EXACTLY ONE `[[QUIZ q=<n> of=5 pts=<0-2> max=2 cat=<AO>]]` marker per question — five in total, one for each of Q1–Q5, every one with `max=2` (the quiz is 5 questions × 2 marks = 10). Never skip a question's marker, never merge two, never use a max other than 2. The SERVER sums these to produce the score; a missing or wrong marker makes the score wrong.\n\n"
                        . "### THE SCORE IS THE CARD'S JOB — STATE NO NUMBER (one source of truth)\n"
                        . "Do NOT write a score, mark, X/Y, percentage, or grade NUMBER anywhere in the chat — not in the results message, not in any dashboard you render. The Quiz Result card beside the chat is the SINGLE authoritative source for the figures (the server computes them from your markers). In chat, speak only qualitatively about performance (e.g. \"a clean round — every answer landed\", or \"strong, with two to review\") and point the student to the result card for their score and grade. If the protocol's dashboard template above contains a `Score: [..]/10` or `[percentage]%` or `Grade [..]` line, OMIT that numeric line — replace it with the qualitative comment.\n";
                }
                return $content;
            }
        ...
        if (strpos($task, 'cw_step_') === 0 || strpos($task, 'cw_trial_') === 0) {
            $cw_protocol_map = [
                ...
                // ⭐ v7.20.451 (2026-08-05) — THE +1 RENUMBER. Neil inserted a new Step 8
                // ("Update Your Plot — Values") between Step 7 and Scene Selection, and chose to
                // WELD the task keys to the titles rather than freeze them, so every step from the
                // old 8 upward moved. The LearnDash course was renumbered first (staging, course
                // 41165, new topic 58666); this map follows it. Prod is NOT renumbered yet —
                // see the guard in deploy-production.sh, which refuses a split ship.
                'cw_step_8'  => 'CW-STEP-08-update-plot-values.md',
                'cw_step_9'  => 'CW-STEP-09-scene-selection.md',
                'cw_step_10' => 'CW-STEP-10-draft-1-prose-style.md',
                'cw_step_11' => 'CW-STEP-11-character-profile.md',
                'cw_step_12' => 'CW-STEP-12-update-plot-goals.md',
                'cw_step_13' => 'CW-STEP-13-draft-2-character-arc.md',
                'cw_step_14' => 'CW-STEP-14-character-archetypes.md',
                'cw_step_15' => 'CW-STEP-15-update-plot-archetypes.md',
                'cw_step_16' => 'CW-STEP-16-draft-3-archetypes.md',
                'cw_step_17' => 'CW-STEP-17-deepen-empathy.md',
                'cw_step_18' => 'CW-STEP-18-update-plot-empathy.md',
                'cw_step_19' => 'CW-STEP-19-draft-4-empathy.md',
                'cw_step_20' => 'CW-STEP-20-theme-tone.md',
                'cw_step_21' => 'CW-STEP-21-update-plot-theme.md',
                'cw_step_22' => 'CW-STEP-22-draft-5-theme-tone.md',
                'cw_step_23' => 'CW-STEP-23-genre.md',
                'cw_step_24' => 'CW-STEP-24-update-plot-genre.md',
                'cw_step_25' => 'CW-STEP-25-draft-6-genre.md',
                'cw_step_26' => 'CW-STEP-26-structural-elements.md',
                'cw_step_27' => 'CW-STEP-27-update-plot-structural.md',
                'cw_step_28' => 'CW-STEP-28-draft-7-structural-elements.md',
                'cw_step_29' => 'CW-STEP-29-final-draft-spag.md',
                'cw_step_30' => 'CW-STEP-30-metacognitive-reflection.md',
        ...
        // just the step it owes and cannot batch later steps it can no longer see. When
        // NOT segmented (setup / Q1 / Q3 / Q4 / Q5 — no step file yet), the strip does
        // not run, so the whole procedure stays as the fallback and those questions are
        // untouched. This is the seam that makes Q2 desegment while the un-built
        // questions keep working; the final increment removes the procedure from
        // `always` for good once every question is segmented.
        if ($is_segmented_q) {
            $files = array_values(array_filter($files, function ($f) {
                return strpos($f, 'protocol-a-assessment.md') === false
                    && strpos($f, 'protocol-q1-msq.md') === false;
            }));
        }
...
TEMPLATE;
    ...
    public function build_preamble($context, $user_id) {
        ...
        if (strpos($task, 'cw_step_') === 0 || strpos($task, 'cw_trial_') === 0) {
            $cw_step_labels = [
                ...
                // v7.20.451 — shifted +1 with the map above. Both maps move together or the
                // sidebar silently mislabels a step it is loading correctly.
                'cw_step_7' => 'Universal Values', 'cw_step_8' => 'Update Plot: Values',
                'cw_step_9' => 'Scene Selection', 'cw_step_10' => 'Draft 1: Prose Style',
                'cw_step_11' => 'Character Profile', 'cw_step_12' => 'Update Plot: Goals',
                'cw_step_13' => 'Draft 2: Character Arc', 'cw_step_14' => 'Character Archetypes',
                'cw_step_15' => 'Update Plot: Archetypes', 'cw_step_16' => 'Draft 3: Archetypes',
                'cw_step_17' => 'Deepen Empathy', 'cw_step_18' => 'Update Plot: Empathy',
                'cw_step_19' => 'Draft 4: Empathy', 'cw_step_20' => 'Theme & Tone',
                'cw_step_21' => 'Update Plot: Theme', 'cw_step_22' => 'Draft 5: Theme & Tone',
                'cw_step_23' => 'Genre', 'cw_step_24' => 'Update Plot: Genre',
                'cw_step_25' => 'Draft 6: Genre', 'cw_step_26' => 'Structural Elements',
                'cw_step_27' => 'Update Plot: Structural', 'cw_step_28' => 'Draft 7: Structural',
                'cw_step_29' => 'Final Draft — SPAG', 'cw_step_30' => 'Metacognitive Reflection',
    ...
    public function assessment_beat_sequence($context) {
        ...
        return [
            ...
            // v7.19.299 (Increment 1): per-paragraph self-rate/targeting files (the
            // monolith's ¶1 + ¶2 wordings differ → verbatim demands separate files).
            // Mark / feedback / gold are SHARED files, loaded on BOTH paragraphs —
            // ¶2 reuses ¶1's spans exactly as the monolith said ("same structure as
            // Paragraph 1"), now made literal so every paragraph gets mark+feedback+
            // both golds (universal granular rule). Gate beat removed: the ¶1→¶2 hand-off
            ...
            // v7.19.360 (FIX F): detects re-tightened after the 11:30 run. The
            // v354 broadening over-corrected: `Marks?[\s]Paragraph 1` matched the
            // conversational "Let's now mark Paragraph 1 in full" (a false
            // advance — the real marking turn then ran on the FEEDBACK slice and
            // improvised a holistic rubric), and the loosened gold detect
            // accepted a SINGLE freeform gold, so the ¶1 gold slice (step 6) was
            // never served and the both-models mandate never enforced. Rules:
            ...
            // line, which the old detect never matched. v7.19.368: 'Unit 1 —
            // Source A' heading alternation removed — gold rewrites use the same
            // unit headings, so ¶1's golds fired ¶2's mark detect through the
            // ask-skip probe and skipped ¶2's self-rate/targeting (corpus replay).
            ['id' => 'q2_p1_mark',     'step' => 4,  'label' => '¶1 Mark',      'group' => 'Question 2', 'file' => 'modules/assessment-steps/a-q2-mark.md',        'type' => 'produce', 'detect' => '/Paragraph 1 \(4 marks|Paragraph 1[^\n]{0,20}(?:sub)?total[^\n]{0,20}\d|Marks?:\s*Paragraph 1|Paragraph 1 (?:raw |final )?score[:\s]|(?:Your|Final) paragraph score[:\s]/i'],
            ...
            ['id' => 'q2_p2_mark',     'step' => 9,  'label' => '¶2 Mark',      'group' => 'Question 2', 'file' => 'modules/assessment-steps/a-q2-mark.md',        'type' => 'produce', 'detect' => '/Paragraph 2 \(4 marks|Paragraph 2[^\n]{0,20}(?:sub)?total[^\n]{0,20}\d|Marks?:\s*Paragraph 2|Paragraph 2 (?:raw |final )?score[:\s]|(?:Your|Final) paragraph score[:\s]/i'],
            ...
            // Q3 — 3 TTECEA body paragraphs (v7.19.377, Increment 2). Same slice
            // discipline as Q2 (FIX H): intake rides BP1's self-rate slice; the
            // feedback slice ends with the gold Pause gate; gold is produce-now
            // with BP-aware tails. Mark detects are score-line anchored (FIX F:
            // never bare "mark Body Paragraph N" prose); gold detect needs BOTH
            // canonical labels with the gate-menu negative lookaheads; the
            // summary line "overall Question 3 score: X out of 12" doubles as
            // the existing server mark harvest — unchanged wording.
            ['id' => 'q3_bp1_selfrate', 'step' => 13, 'label' => 'BP1 Self-rate', 'group' => 'Question 3', 'file' => 'modules/assessment-steps/a-q3-bp1-selfrate.md', 'type' => 'ask',     'detect' => '/Goal Achievement|scale of 1.?5|perceptive depth \(AO2\)/i'],
            ['id' => 'q3_bp1_targeting','step' => 14, 'label' => 'BP1 Targeting', 'group' => 'Question 3', 'file' => 'modules/assessment-steps/a-q3-targeting.md',     'type' => 'ask',     'detect' => '/Assessment Objective Targeting|strongest AO2|which specific TTECEA element/i'],
            ['id' => 'q3_bp1_mark',     'step' => 15, 'label' => 'BP1 Mark',      'group' => 'Question 3', 'file' => 'modules/assessment-steps/a-q3-mark.md',          'type' => 'produce', 'detect' => '/Body Paragraph 1 score[:\s\*]|Body Paragraph 1[^\n]{0,20}\(4 marks|BP\s*1[^\n]{0,20}(?:sub)?total[^\n]{0,20}\d/i'],
            ['id' => 'q3_bp1_feedback', 'step' => 16, 'label' => 'BP1 Feedback',  'group' => 'Question 3', 'file' => 'modules/assessment-steps/a-q3-feedback.md',      'type' => 'produce', 'detect' => '/How to Improve|My Assessment[:\s\*]|Where marks were lost|What you did well/i'],
            ['id' => 'q3_bp1_gold',     'step' => 17, 'label' => 'BP1 Gold',      'group' => 'Question 3', 'file' => 'modules/assessment-steps/a-q3-gold.md',          'type' => 'produce', 'detect' => '/Your (?:Body )?Paragraph (?:[123]\s+)?Rewritten to Gold Standard(?!\s*level)[\s\S]*Optimal Gold Standard Model(?!\s+for comparison)|Optimal Gold Standard Model(?!\s+for comparison)[\s\S]*Your (?:Body )?Paragraph (?:[123]\s+)?Rewritten to Gold Standard(?!\s*level)/i'],
            ['id' => 'q3_bp2_selfrate', 'step' => 18, 'label' => 'BP2 Self-rate', 'group' => 'Question 3', 'file' => 'modules/assessment-steps/a-q3-bpn-selfrate.md', 'type' => 'ask',     'detect' => '/Goal Achievement|scale of 1.?5|perceptive depth \(AO2\)/i'],
            ['id' => 'q3_bp2_targeting','step' => 19, 'label' => 'BP2 Targeting', 'group' => 'Question 3', 'file' => 'modules/assessment-steps/a-q3-targeting.md',     'type' => 'ask',     'detect' => '/Assessment Objective Targeting|strongest AO2|which specific TTECEA element/i'],
            ['id' => 'q3_bp2_mark',     'step' => 20, 'label' => 'BP2 Mark',      'group' => 'Question 3', 'file' => 'modules/assessment-steps/a-q3-mark.md',          'type' => 'produce', 'detect' => '/Body Paragraph 2 score[:\s\*]|Body Paragraph 2[^\n]{0,20}\(4 marks|BP\s*2[^\n]{0,20}(?:sub)?total[^\n]{0,20}\d/i'],
            ['id' => 'q3_bp2_feedback', 'step' => 21, 'label' => 'BP2 Feedback',  'group' => 'Question 3', 'file' => 'modules/assessment-steps/a-q3-feedback.md',      'type' => 'produce', 'detect' => '/How to Improve|My Assessment[:\s\*]|Where marks were lost|What you did well/i'],
            ['id' => 'q3_bp2_gold',     'step' => 22, 'label' => 'BP2 Gold',      'group' => 'Question 3', 'file' => 'modules/assessment-steps/a-q3-gold.md',          'type' => 'produce', 'detect' => '/Your (?:Body )?Paragraph (?:[123]\s+)?Rewritten to Gold Standard(?!\s*level)[\s\S]*Optimal Gold Standard Model(?!\s+for comparison)|Optimal Gold Standard Model(?!\s+for comparison)[\s\S]*Your (?:Body )?Paragraph (?:[123]\s+)?Rewritten to Gold Standard(?!\s*level)/i'],
            ['id' => 'q3_bp3_selfrate', 'step' => 23, 'label' => 'BP3 Self-rate', 'group' => 'Question 3', 'file' => 'modules/assessment-steps/a-q3-bpn-selfrate.md', 'type' => 'ask',     'detect' => '/Goal Achievement|scale of 1.?5|perceptive depth \(AO2\)/i'],
            ['id' => 'q3_bp3_targeting','step' => 24, 'label' => 'BP3 Targeting', 'group' => 'Question 3', 'file' => 'modules/assessment-steps/a-q3-targeting.md',     'type' => 'ask',     'detect' => '/Assessment Objective Targeting|strongest AO2|which specific TTECEA element/i'],
            ['id' => 'q3_bp3_mark',     'step' => 25, 'label' => 'BP3 Mark',      'group' => 'Question 3', 'file' => 'modules/assessment-steps/a-q3-mark.md',          'type' => 'produce', 'detect' => '/Body Paragraph 3 score[:\s\*]|Body Paragraph 3[^\n]{0,20}\(4 marks|BP\s*3[^\n]{0,20}(?:sub)?total[^\n]{0,20}\d/i'],
            ['id' => 'q3_bp3_feedback', 'step' => 26, 'label' => 'BP3 Feedback',  'group' => 'Question 3', 'file' => 'modules/assessment-steps/a-q3-feedback.md',      'type' => 'produce', 'detect' => '/How to Improve|My Assessment[:\s\*]|Where marks were lost|What you did well/i'],
            ['id' => 'q3_bp3_gold',     'step' => 27, 'label' => 'BP3 Gold',      'group' => 'Question 3', 'file' => 'modules/assessment-steps/a-q3-gold.md',          'type' => 'produce', 'detect' => '/Your (?:Body )?Paragraph (?:[123]\s+)?Rewritten to Gold Standard(?!\s*level)[\s\S]*Optimal Gold Standard Model(?!\s+for comparison)|Optimal Gold Standard Model(?!\s+for comparison)[\s\S]*Your (?:Body )?Paragraph (?:[123]\s+)?Rewritten to Gold Standard(?!\s*level)/i'],
            ['id' => 'q3_summary',      'step' => 28, 'label' => 'Q3 Summary',    'group' => 'Question 3', 'file' => 'modules/assessment-steps/a-q3-summary.md',       'type' => 'produce', 'detect' => '/overall Question 3 score|Question 3 Complete Assessment|Question 3 Final Summary/i'],
            // Q4 — Intro(/2) + 3 comparative BPs(/4) + Conclusion(/2) (v7.19.378,
            // Increment 3). ONE reflection pair before the BPs (monolith stores a
            // single q4_body_self_rating); intro + conclusion get their own
            // mark→gold pairs with distinct gold labels ("…Rewritten to Gold
            // Standard" + "Optimal Gold Standard Introduction/Conclusion").
            ['id' => 'q4_intro_mark',  'step' => 29, 'label' => 'Intro Mark',     'group' => 'Question 4', 'file' => 'modules/assessment-steps/a-q4-intro.md',      'type' => 'produce', 'detect' => '/Introduction score[:\s\*]|Introduction[^\n]{0,20}\(2 marks/i'],
            ['id' => 'q4_intro_gold',  'step' => 30, 'label' => 'Intro Gold',     'group' => 'Question 4', 'file' => 'modules/assessment-steps/a-q4-intro-gold.md', 'type' => 'produce', 'detect' => '/Your Introduction Rewritten to Gold Standard[\s\S]*Optimal Gold Standard Introduction|Optimal Gold Standard Introduction[\s\S]*Your Introduction Rewritten to Gold Standard/i'],
            ['id' => 'q4_selfrate',    'step' => 31, 'label' => 'Self-rate',      'group' => 'Question 4', 'file' => 'modules/assessment-steps/a-q4-selfrate.md',   'type' => 'ask',     'detect' => '/Goal Achievement|scale of 1.?5|comparing writers\'? methods/i'],
            ['id' => 'q4_targeting',   'step' => 32, 'label' => 'Targeting',      'group' => 'Question 4', 'file' => 'modules/assessment-steps/a-q4-targeting.md',  'type' => 'ask',     'detect' => '/Assessment Objective Targeting|HOW writers present ideas|comparative approach with specific example/i'],
            ['id' => 'q4_bp1_mark',    'step' => 33, 'label' => 'BP1 Mark',       'group' => 'Question 4', 'file' => 'modules/assessment-steps/a-q4-mark.md',       'type' => 'produce', 'detect' => '/Body Paragraph 1 score[:\s\*]|Body Paragraph 1[^\n]{0,20}\(4 marks/i'],
            ['id' => 'q4_bp1_feedback','step' => 34, 'label' => 'BP1 Feedback',   'group' => 'Question 4', 'file' => 'modules/assessment-steps/a-q4-feedback.md',   'type' => 'produce', 'detect' => '/How to Improve|My Assessment[:\s\*]|Where marks were lost|What you did well/i'],
            ['id' => 'q4_bp1_gold',    'step' => 35, 'label' => 'BP1 Gold',       'group' => 'Question 4', 'file' => 'modules/assessment-steps/a-q4-gold.md',       'type' => 'produce', 'detect' => '/Your (?:Body )?Paragraph (?:[123]\s+)?Rewritten to Gold Standard(?!\s*level)[\s\S]*Optimal Gold Standard Model(?!\s+for comparison)|Optimal Gold Standard Model(?!\s+for comparison)[\s\S]*Your (?:Body )?Paragraph (?:[123]\s+)?Rewritten to Gold Standard(?!\s*level)/i'],
            ['id' => 'q4_bp2_mark',    'step' => 36, 'label' => 'BP2 Mark',       'group' => 'Question 4', 'file' => 'modules/assessment-steps/a-q4-mark.md',       'type' => 'produce', 'detect' => '/Body Paragraph 2 score[:\s\*]|Body Paragraph 2[^\n]{0,20}\(4 marks/i'],
            ['id' => 'q4_bp2_feedback','step' => 37, 'label' => 'BP2 Feedback',   'group' => 'Question 4', 'file' => 'modules/assessment-steps/a-q4-feedback.md',   'type' => 'produce', 'detect' => '/How to Improve|My Assessment[:\s\*]|Where marks were lost|What you did well/i'],
            ['id' => 'q4_bp2_gold',    'step' => 38, 'label' => 'BP2 Gold',       'group' => 'Question 4', 'file' => 'modules/assessment-steps/a-q4-gold.md',       'type' => 'produce', 'detect' => '/Your (?:Body )?Paragraph (?:[123]\s+)?Rewritten to Gold Standard(?!\s*level)[\s\S]*Optimal Gold Standard Model(?!\s+for comparison)|Optimal Gold Standard Model(?!\s+for comparison)[\s\S]*Your (?:Body )?Paragraph (?:[123]\s+)?Rewritten to Gold Standard(?!\s*level)/i'],
            ['id' => 'q4_bp3_mark',    'step' => 39, 'label' => 'BP3 Mark',       'group' => 'Question 4', 'file' => 'modules/assessment-steps/a-q4-mark.md',       'type' => 'produce', 'detect' => '/Body Paragraph 3 score[:\s\*]|Body Paragraph 3[^\n]{0,20}\(4 marks/i'],
            ['id' => 'q4_bp3_feedback','step' => 40, 'label' => 'BP3 Feedback',   'group' => 'Question 4', 'file' => 'modules/assessment-steps/a-q4-feedback.md',   'type' => 'produce', 'detect' => '/How to Improve|My Assessment[:\s\*]|Where marks were lost|What you did well/i'],
            ['id' => 'q4_bp3_gold',    'step' => 41, 'label' => 'BP3 Gold',       'group' => 'Question 4', 'file' => 'modules/assessment-steps/a-q4-gold.md',       'type' => 'produce', 'detect' => '/Your (?:Body )?Paragraph (?:[123]\s+)?Rewritten to Gold Standard(?!\s*level)[\s\S]*Optimal Gold Standard Model(?!\s+for comparison)|Optimal Gold Standard Model(?!\s+for comparison)[\s\S]*Your (?:Body )?Paragraph (?:[123]\s+)?Rewritten to Gold Standard(?!\s*level)/i'],

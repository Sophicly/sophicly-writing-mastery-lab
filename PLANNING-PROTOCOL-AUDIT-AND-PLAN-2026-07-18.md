...
- Resume fade base: L1 default, L2 only when a filed same-type sibling resolved at ≥L3. · Judge safe-default `weak`: accept (no re-ask step). · Wallet scope: per-doc-lifetime, derive, no blob. · L4: uncapped. · §9.2 override-vs-feed: **feed** (change 2 mostly dissolves it — no menu option touches the rung). · Per-paper ceiling: **4** (Neil 2026-07-18 — overrides the earlier 3; Law 7 swept to match). · Corrections: default ALL to wise-feedback framing (skip confidence detection).
